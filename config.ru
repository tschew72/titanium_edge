...
get '/logout' do
  ...
  post '/newskill' do
    userprofile = env['warden'].user
    newskill = SkillSummary.first_or_create({:skillid => params["skillid"]}).update(:skillrank => params["skillrank"], :user_id => userprofile.id, :status =>2)  #If similar skillID detected, just update it with new set of data.
        {:responsemsg => "New skill added!" }.to_json
  end

  post '/newlanguage' do
    userprofile = env['warden'].user
    newlanguage = TmeSkrLanguage.first_or_create({:skr_lang => params["skr_lang"]},{:skr_lang_speakskill => params["skr_lang_speakskill"], :skr_lang_writeskill => params["skr_lang_writeskill"], :user_id => userprofile.id, :skr_status =>2}).update(:skr_lang_speakskill => params["skr_lang_speakskill"], :skr_lang_writeskill => params["skr_lang_writeskill"], :user_id => userprofile.id, :skr_status =>2)  
